...
class LSTMForClassification(nn.Module):
    ...
    def forward(self, _input, perturbed_input=None, mix_ratio=None, feature_mixup=True):
        ...
        if perturbed_input is None:
            ...
            if feature_mixup:
                emb_noraml = self.emb_layer(_input.t())
                emb_perturb = self.emb_layer(perturbed_input.t())
                m = mix_ratio
                ...
                output_normal, hidden_normal = self.encoder(emb_noraml)
                output_normal = torch.max(output_normal, dim=0)[0]

                output_perturb, hidden_perturb = self.encoder(emb_perturb)
                output_perturb = torch.max(output_perturb, dim=0)[0]
