...
class HuggingFaceModelWrapper(PyTorchModelWrapper):
    ...
    def __init__(self, model, tokenizer, batch_size=32):
        self.model = model.to(train.shared.utils.device)
        if isinstance(tokenizer, transformers.PreTrainedTokenizer):
            tokenizer = train.models.tokenizers.AutoTokenizer(tokenizer=tokenizer)
    ...
    def __call__(self, text_input_list):
        ...
        with torch.no_grad():
            outputs = train.shared.utils.batch_model_predict(
    ...
    def get_grad(self, text_input):
        ...
        """
        if isinstance(self.model, train.models.helpers.T5ForTextToText):
    ...
    def get_pred(self, text_input, meta=None):
        ids = self.encode(text_input)
        predictions = self._model_predict(ids)
    ...
    def get_prob(self, text_input, meta=None):
        ids = self.encode(text_input)
        predictions = self._model_predict(ids)
